...
class Endpoints:
    def __init__(self, app, plugin_json: str, cache_duration: int, page_size: int):
    ...
    async def get_plugins_html_table(
        self, columns: str | None = None, render_link: bool | None = False, classes: str | None = None
    ) -> HTMLResponse:
    ...
    async def get_all_tags(self) -> Dict[int, str]:
        ...
        for plugin_data in self.cache["plugins"]:
            ...
            if isinstance(tags, str):
                ...
                all_tags.update([item for tag in tags for item in tag.split(",")])

        tags_dict = {i: tag.strip() for i, tag in enumerate(all_tags)}

        return tags_dict

    async def get_plugins_by_tag(self, tag_name: str, page: int = 1, page_size: int = 0) -> Dict[str, Any]:
        if page_size == 0:
            page_size = self.page_size
    ...
    @staticmethod
    def filter_plugins_by_names(plugins: List, excluded: Set[str]) -> List[Dict[str, Any]]:
        return [plugin_data for plugin_data in plugins if plugin_data.get("name") not in excluded]

    async def exclude_plugins(
        self, page: int = 1, page_size: int = 10, excluded: List[str] = Body(..., embed=True)
    ) -> Dict[str, Any]:
        # Check if cache is still valid, otherwise update the cache
        if not is_cache_valid(self.cache_duration, self.cache_timestamp):
            await self.cache_plugins()
    ...
    async def get_plugins_by_author(
        self, author_name: str = Body(..., embed=True), page: int = 1, page_size: int = 0
    ) -> Dict[str, Any]:
        if page_size == 0:
            page_size = self.page_size
    ...
    async def download_plugin_zip(self, plugin_data: dict = Body({"url": ""})):
        ...
        async with AsyncClient() as client:
            ...
            i = 0
            try:
                # list of assets with files different from Source code zips
    ...
    @staticmethod
    async def download_releases_plugin_zip(plugin_name: str, url_zip: str, version_origin: str):
        # Define a cache directory
        cache_dir = "zip_cache"
        if not os.path.exists(cache_dir):
            os.makedirs(cache_dir)
